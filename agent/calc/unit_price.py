...
from quantity import compute as compute_quantity  # noqa: E402
...
def exact_decimal(value: Fraction, what: str) -> Decimal:
    """끝나는 소수만 Decimal로 옮긴다. 끝나지 않으면 금액 표시·반올림 규칙이 정해지지 않았으므로 계산하지 않는다."""
    d = value.denominator
    for p in (2, 5):
        while d % p == 0:
            d //= p
    if d != 1:
        raise RuntimeError(f"{what} {value.numerator}/{value.denominator}는 끝나는 소수가 아니라 금액을 계산하지 않습니다"
                           "(금액 반올림 규칙 미정).")
...
def apply_prices(labor: dict, rates: dict, search_index=None, query: str | None = None) -> dict:
    ...
    if labor["section_no"] != case["section_no"] or labor["conditions"] != quantity_conditions():
    ...
    for trade in case["trades"]:
        line = lines[trade]
        person_days = exact_decimal(Fraction(line["quantity"]["exact"]), f"{trade} 노무량")
        crew, output = Decimal(line["basis"]["crew"]), Decimal(line["basis"]["daily_output"])
        per_m3 = exact_decimal(Fraction(crew) / Fraction(output), f"{trade} 1㎥당 노무량")
