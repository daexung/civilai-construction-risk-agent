...
품량을 몇 자리에서 반올림할지(단가를 곱하기 전 적용 자릿수)는 정하지 않았다. 금액은 이 계산기가 다루지 않는다
(품셈 1-2-2 금액 버림 규칙과 남은 항목은 unit_price.AMOUNT_POLICY).
...
ROUNDING = "반올림·절사 없음. 정확값은 기약분수 또는 끝나는 소수. 품량 표시·적용 자릿수는 정하지 않았다. 금액 버림은 이 계산기 범위 밖이다."
