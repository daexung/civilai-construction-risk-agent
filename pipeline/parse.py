...
# 사용하므로 그대로 두고, 기존 규칙에서 본문이 없던 표에만 아래 규칙을 적용한다.
...
def table_header_count(table: list) -> tuple[int, str | None]:
    original = count_header_rows(table)
    if original < len(table):
        return original, None
    for index, row in enumerate(table):
        if any(is_table_value_cell(cell) for cell in row):
            return index, "header_extended_numeric"
    # 숫자가 없는 Type/적용기준 표도 첫 줄만 머리글로 두고 내용을 보존한다.
    return min(1, len(table)), "header_fallback"
...
def table_to_records(table: list, section_title: str, page_no: int) -> list:
    header_count, header_issue = table_header_count(table)
    ...
    for row in table[header_count:]:
        ...
        if label == "비고":
            ...
            records.append({"text": text, "section": section_title, "page": page_no,
                            "structure": {"status": "uncertain" if header_issue else "ok",
                                          "issues": [header_issue] if header_issue else []}})
            continue
